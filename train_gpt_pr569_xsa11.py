...
class Hyperparameters:
    ...
    eval_temperature = float(os.environ.get("EVAL_TEMPERATURE", 0.98))  # post-quant logit temperature
...
def main():
    ...
    log0(f"v42: 11L LeakyReLU(0.5)² Late-QAT@{args.late_qat_threshold} int6-all FullGPTQ EMA({args.ema_decay}) TightSWA XSA-all({args.xsa_last_n}) PartialRoPE({args.rope_dims}/64) LNScale VE128 SmearGate BigramHash({args.bigram_vocab_size}) QATalign({args.qat_clip_pct}) VRL Prune({args.prune_pct}) T={args.eval_temperature} RawBinary")
