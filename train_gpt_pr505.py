...
class Hyperparameters:
    ...
    # Value Embeddings: reinject token identity into attention values at deep layers
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear_gate,bigram,skip_gates,ve_shared,ve_layer_scales",
    ).split(",")
    if pattern
)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        # Add value embeddings to v before attention if provided
        if v_embed is not None:
            ve_reshaped = v_embed.reshape(bsz, seqlen, self.num_kv_heads, self.head_dim).transpose(1, 2)
            v = v + ve_reshaped
...
class GPT(nn.Module):
    def __init__(
        ...
        self.bigram_emb = BigramHashEmbedding(bigram_buckets, bigram_embed_dim, model_dim) if bigram_buckets > 0 else None
        ...
        # Value Embeddings: reinject token identity into values at deep layers
...
def main() -> None:
    ...
    other_scalar_params = [base_model.smear_gate.gate]
    if base_model.bigram_emb is not None:
        other_scalar_params.append(base_model.bigram_emb.embed.weight)
    ...
    if base_model.ve_shared is not None:
        other_scalar_params.extend(list(base_model.ve_shared.parameters()))
        other_scalar_params.extend(list(base_model.ve_layer_scales.parameters()))
