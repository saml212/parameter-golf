...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.attn_gate = None  # set by GPT.__init__
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        if self.attn_gate is not None:
            ...
            y = y * gate.unsqueeze(-1)  # (bsz, seqlen, num_heads, head_dim)
...
class GPT(nn.Module):
    def __init__(
        ...
        if gated_attention:
            for block in self.blocks:
                block.attn.attn_gate = nn.Linear(model_dim, num_heads, bias=False)
                nn.init.zeros_(block.attn.attn_gate.weight)  # init as identity (sigmoid(0) = 0.5)
        self._init_weights()
...
def main() -> None:
    ...
    ga_params = sum(p.numel() for n, p in base_model.named_parameters() if 'attn_gate' in n)
    log0(f"gated_attention:enabled={args.gated_attention} params:{ga_params}")
