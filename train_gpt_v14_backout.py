...
class Hyperparameters:
    ...
    backout = bool(int(os.environ.get("BACKOUT", "1")))
...
class GPT(nn.Module):
    def __init__(
        ...
        # Backout Connection: subtract mid-layer hidden state from final output
        self.backout_lambda = nn.Parameter(torch.tensor([backout_init], dtype=torch.float32)) if backout else None
        self._backout_layer = num_layers // 2
        self._init_weights()
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        skips: list[Tensor] = []
        ve_cache: dict = {}
        x_backout = torch.zeros_like(x)  # placeholder for backout
        for i in range(self.num_encoder_layers):
            ve = self._get_ve(i, input_ids, ve_cache)
            x = self.blocks[i](x, x0, v_embed=ve)
            if i == self._backout_layer:
                x_backout = x
            skips.append(x)
...
def main() -> None:
    ...
    log0(f"backout:enabled={args.backout} init={args.backout_init} layer={base_model._backout_layer}")
    ...
    eval_model = GPT(
        ...
        backout=args.backout, backout_init=args.backout_init,
    ).to(device).bfloat16()
