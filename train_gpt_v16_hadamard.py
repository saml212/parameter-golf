...
class Hyperparameters:
    ...
    xsa_last_n = int(os.environ.get("XSA_LAST_N", 11))  # XSA on all layers (our best config)
    ...
    hadamard_quant = bool(int(os.environ.get("HADAMARD_QUANT", "1")))
...
def _fast_walsh_hadamard(x: Tensor) -> Tensor:
    """In-place Fast Walsh-Hadamard Transform along dim=0. Input shape: (n, ...) where n is power of 2."""
    n = x.shape[0]
    h = 1
    while h < n:
        for i in range(0, n, h * 2):
            ...
            x[i+h:i+2*h] = a - b
        h *= 2
    x /= n ** 0.5  # Normalize so H is orthogonal (H^T H = I)
    return x

def hadamard_rotate_rows(t: Tensor) -> Tensor:
    """Apply Walsh-Hadamard rotation to rows of a 2D tensor. Rows must be power-of-2."""
    if t.ndim != 2 or not _is_power_of_2(t.shape[0]):
        return t  # Skip non-eligible tensors
    return _fast_walsh_hadamard(t.clone())

def hadamard_unrotate_rows(t: Tensor) -> Tensor:
    """Inverse Walsh-Hadamard = same as forward (H is symmetric and orthogonal)."""
    if t.ndim != 2 or not _is_power_of_2(t.shape[0]):
        return t
    return _fast_walsh_hadamard(t.clone())
...
def quantize_int6_per_row(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    ...
    return q, scale
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], hadamard: bool = False):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            t_quant = t
            rotated = False
            if hadamard and t.ndim == 2 and _is_power_of_2(t.shape[0]):
                t_quant = hadamard_rotate_rows(t)
                rotated = True
            q, s = quantize_int6_per_row(t_quant)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6", "hadamard": rotated}
        else:
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if isinstance(info, dict) and info.get("hadamard", False):
            deq = hadamard_unrotate_rows(deq)
...
def main() -> None:
    ...
    quant_result, quant_meta = mixed_quantize_int6(sd_cpu, {"mlp", "attn"}, hadamard=args.hadamard_quant)
    if args.hadamard_quant:
        h_count = sum(1 for v in quant_meta.values() if isinstance(v, dict) and v.get("hadamard", False))
        log0(f"hadamard_quant: rotated {h_count} weight matrices")
