...
class Hyperparameters:
    ...
    value_residual = bool(int(os.environ.get("VALUE_RESIDUAL", "1")))  # VRL: cache layer 0 V, mix into all layers
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor]:
        ...
        # Value Residual: mix layer-0 V into subsequent layers
...
class Block(nn.Module):
    def __init__(
        ...
        if dtg:
            ...
            self.dtg_gate = None
    def forward(self, x: Tensor, x0: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor]:
        mix = self.resid_mix.to(dtype=x.dtype)
        x_in = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        attn_out, v_out = self.attn(self.attn_norm(x_in) * self.ln_scale_factor, v_embed=v_embed, v0=v0)
...
class GPT(nn.Module):
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    ln_scale=ln_scale,
                    dtg=dtg,
                    value_residual=(value_residual and i > 0),  # Layer 0 is the V cache source, no VRL
                )
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        skips: list[Tensor] = []
        ve_cache: dict = {}
        # VRL: Run layer 0 first to get v0, then use it for all subsequent layers
        ve = self._get_ve(0, input_ids, ve_cache)
        x, v0 = self.blocks[0](x, x0, v_embed=ve)  # Layer 0: no v0 kwarg (value_residual=False)
        skips.append(x)
        for i in range(1, self.num_encoder_layers):
            ve = self._get_ve(i, input_ids, ve_cache)
            x, _ = self.blocks[i](x, x0, v_embed=ve, v0=v0)
            skips.append(x)
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        ve_cache: dict = {}
        ve = self._get_ve(0, input_ids, ve_cache)
        x, v0 = self.blocks[0](x, x0, v_embed=ve)
        skips.append(x)
        for i in range(1, self.num_encoder_layers):
            ve = self._get_ve(i, input_ids, ve_cache)
            x, _ = self.blocks[i](x, x0, v_embed=ve, v0=v0)
...
def main() -> None:
    ...
    vrl_params = sum(p.numel() for n, p in base_model.named_parameters() if 'vr_lambda' in n)
    log0(f"VRL:enabled={args.value_residual} vrl_params:{vrl_params}")
