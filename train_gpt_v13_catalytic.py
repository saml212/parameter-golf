...
class Hyperparameters:
    ...
    catalytic = bool(int(os.environ.get("CATALYTIC", "1")))  # Catalytic residuals: x + c*f(x)
...
class Block(nn.Module):
    def __init__(
        ...
        # Catalytic Residuals: x + c*f(x) where c is learned per-dimension (PR #450)
        if catalytic:
    ...
    def forward(self, x: Tensor, x0: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        if self._catalytic:
            ca = self.catalytic_attn.to(dtype=x_in.dtype)[None, None, :]
            x_out = x_in + self.attn_scale.to(dtype=x_in.dtype)[None, None, :] * ca * attn_out
        else:
            x_out = x_in + self.attn_scale.to(dtype=x_in.dtype)[None, None, :] * attn_out
        if self._catalytic:
            cm = self.catalytic_mlp.to(dtype=x_out.dtype)[None, None, :]
            x_out = x_out + self.mlp_scale.to(dtype=x_out.dtype)[None, None, :] * cm * self.mlp(self.mlp_norm(x_out) * self.ln_scale_factor)
        else:
...
def main() -> None:
    ...
    cat_params = sum(p.numel() for n, p in base_model.named_parameters() if 'catalytic' in n)
    log0(f"catalytic:enabled={args.catalytic} params:{cat_params}")
