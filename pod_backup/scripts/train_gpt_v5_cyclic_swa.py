...
def main() -> None:
    ...
    cyclic_swa_cycles = int(os.environ.get("CYCLIC_SWA_CYCLES", 0))  # 0 = linear warmdown (default)
    ...
    def lr_mul(step: int, elapsed_ms: float) -> float:
        ...
        # Cyclic SWA: replace linear decay with cosine cycles during warmdown
        if cyclic_swa_cycles > 0 and frac < 1.0:
            # frac goes from 1.0 to 0.0 during warmdown
            # Map to cycle position: each cycle goes 0→1 within the warmdown
            cycle_frac = (1.0 - frac) * cyclic_swa_cycles  # 0 to num_cycles
            cycle_pos = cycle_frac % 1.0  # position within current cycle [0, 1)
            # Cosine annealing within cycle: peak at start (cycle_pos=0), trough at end (cycle_pos=1)
            peak_lr = 0.4  # restart to 40% of max LR
            return peak_lr * 0.5 * (1.0 + math.cos(math.pi * cycle_pos))
    ...
    while True:
        ...
        # For cyclic SWA, collect during warmdown same as linear SWA
        # The diversity comes from the cycling LR schedule
        in_warmdown = scale < 1.0
        if in_warmdown and step % swa_every == 0:
