...
def main() -> None:
    ...
    while True:
        ...
        # Progressive layer freezing: during warmdown, freeze early layers
        layer_freeze = bool(int(os.environ.get("LAYER_FREEZE", 0)))
        if layer_freeze and scale < 1.0:
            warmdown_progress = 1.0 - scale  # 0 at start of warmdown, 1 at end
            num_blocks = len(base_model.blocks)
            freeze_up_to = int(warmdown_progress * num_blocks // 2)
