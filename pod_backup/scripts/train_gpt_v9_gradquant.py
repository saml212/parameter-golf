...
def quantize_float_tensor(t: Tensor, bits: int = 8, per_row_bits: Tensor | None = None) -> tuple[Tensor, Tensor]:
...
def quantize_state_dict_int8(state_dict: dict[str, Tensor], grad_sensitivity: dict[str, Tensor] | None = None):
    ...
    for name, tensor in state_dict.items():
        ...
        # Gradient-guided bit allocation: sensitive tensors get more bits
        stats["num_float_tensors"] += 1
        if grad_sensitivity is not None and name in grad_sensitivity:
            sens = grad_sensitivity[name].mean().item()
            # Top tercile: int8, middle: int6, bottom: int5
            all_sens = sorted([grad_sensitivity[n].mean().item() for n in grad_sensitivity])
            p33 = all_sens[len(all_sens) // 3] if len(all_sens) > 2 else 0
            p66 = all_sens[2 * len(all_sens) // 3] if len(all_sens) > 2 else float('inf')
            if sens >= p66:
                bits = 8
            elif sens >= p33:
                bits = 6
...
def main() -> None:
    ...
    while True:
        ...
        # Gradient-guided quantization: accumulate gradient stats in last 100 steps
        grad_quant_enabled = bool(int(os.environ.get("GRAD_QUANT", 0)))
        if grad_quant_enabled and stop_after_step is not None and step >= stop_after_step - 100:
            if not hasattr(main, '_grad_accum'):
                main._grad_accum = {}
                main._grad_count = 0
            main._grad_count += 1
            for name, p in base_model.named_parameters():
                if p.grad is not None and p.ndim == 2 and p.shape[0] >= 64:
                    gs = p.grad.detach().float().pow(2).mean(dim=1)  # per-row mean squared gradient
                    if name not in main._grad_accum:
                        main._grad_accum[name] = gs
                    else:
                        main._grad_accum[name] += gs
    ...
    # Pass gradient sensitivity for gradient-guided quantization
    grad_sens = getattr(main, '_grad_accum', None)
    if grad_sens:
        for name in grad_sens:
            grad_sens[name] /= max(getattr(main, '_grad_count', 1), 1)
        log0(f"grad_quant: using gradient sensitivity for {len(grad_sens)} tensors")
    quant_obj, quant_stats = quantize_state_dict_int8(base_model.state_dict(), grad_sensitivity=grad_sens)
