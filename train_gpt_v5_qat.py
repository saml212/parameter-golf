...
def quantize_state_dict_int8(state_dict: dict[str, Tensor]):
    ...
    for name, tensor in state_dict.items():
        ...
        # Int5 for MLP weights, int6 for attention — MLP tolerates more aggressive quant
...
_QAT_ENABLED = bool(int(os.environ.get("QAT", "1")))
_QAT_BITS = int(os.environ.get("QAT_BITS", 6))

def fake_quantize_per_row(w: Tensor, bits: int = 6) -> Tensor:
    """STE-based fake quantization: forward pass sees quantized weights, gradients flow through."""
    max_val = (2 ** (bits - 1)) - 1  # int6: 31, int5: 15
    scale = w.detach().abs().amax(dim=-1, keepdim=True).div_(float(max_val)).clamp_(min=1.0 / float(max_val))
    w_deq = (w / scale).round().clamp_(-max_val, max_val) * scale
...
class CastedLinear(nn.Linear):
    ...
    def forward(self, x: Tensor) -> Tensor:
        w = self.weight
        if self._qat and self.training and w.ndim == 2 and w.shape[0] >= 64:
            w = fake_quantize_per_row(w, bits=_QAT_BITS)
...
def main() -> None:
    ...
    while True:
        ...
        # SWA: collect checkpoints every 50 steps during last 50% of warmdown
        if scale < 0.5 and step % 50 == 0:
