...
class Hyperparameters:
    ...
    # Causal online TTT
    causal_ttt = bool(int(os.environ.get("CAUSAL_TTT", 0)))
    causal_ttt_lr = float(os.environ.get("CAUSAL_TTT_LR", 0.01))
...
def eval_val_sliding_causal_ttt(
    ...
    val_tokens: Tensor, base_bytes_lut: Tensor, has_leading_space_lut: Tensor,
    is_boundary_token_lut: Tensor, eval_seq_len: int, eval_stride: int,
    ttt_lr: float = 0.01, batch_seqs: int = 32, log_fn=None,
) -> tuple[float, float]:
    """Causal online TTT: score each chunk, then adapt on scored tokens. Legal per rules."""
    total_tokens = val_tokens.numel() - 1
    all_starts = [s for s in range(0, total_tokens - eval_stride + 1, eval_stride)
                  if min(s + eval_seq_len, total_tokens) - s >= eval_stride]
    n = len(all_starts)
    ...
    # TTT optimizer on all unfrozen params
    ...
    for bi in range(0, len(my_starts), batch_seqs):
        ...
        # STEP 1: Score this batch (inference mode, no gradients)
        base_model.eval()
        with torch.inference_mode():
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                logits = base_model.get_logits(x_batch)
        ...
        # STEP 2: Adapt on the SCORED tokens (causal — only backward-looking)
        base_model.train()
        ttt_optimizer.zero_grad(set_to_none=True)
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            adapt_loss = base_model(x_batch, y_batch)
        adapt_loss.backward()
        if world_size > 1:
            for p in ttt_params:
...
def main() -> None:
    ...
    if args.eval_stride > 0:
        ...
        if args.causal_ttt:
            s_val_loss, s_val_bpb = eval_val_sliding_causal_ttt(
                args, base_model, rank, world_size, device,
                val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
                eval_seq_len=effective_eval_seq_len, eval_stride=args.eval_stride,
                ttt_lr=args.causal_ttt_lr, batch_seqs=args.eval_batch_seqs,
                log_fn=log0,
            )
        else:
        ...
        log0(
            ...
            f"{' causal_ttt_lr:' + str(args.causal_ttt_lr) if args.causal_ttt else ''}"
        )
