...
def eval_val_sliding(logits_fn, rank, world_size, device, val_tokens,
                     ...
                     seq_len, stride, eval_batch_seqs=256, temperature=1.0):
    ...
    with torch.inference_mode():
        for i in range(0, len(my_windows), eval_batch_seqs):
            ...
            for b in range(bs):
                ...
                if temperature != 1.0: sl = sl / temperature
...
def main():
    ...
    # Apply EMA weights — save raw for delta measurement
    log0("ema:applying EMA weights")
    current_state = base_model.state_dict()
    raw_cpu = {k: v.detach().cpu().clone() for k, v in current_state.items()} if rank == 0 else None
    avg_state = {name: t.to(dtype=current_state[name].dtype) for name, t in ema_state.items()}
    ema_cpu = {k: v.detach().cpu().clone() for k, v in avg_state.items()} if rank == 0 else None
    base_model.load_state_dict(avg_state, strict=True)
    # Delta measurement
    if rank == 0 and raw_cpu is not None:
        import zstandard as _zstd
        delta_vals = []
        for k in raw_cpu:
            d = (ema_cpu[k].float() - raw_cpu[k].float())
            delta_vals.append(d.flatten())
        all_delta = torch.cat(delta_vals)
        # Int8 quantize delta
        amax = all_delta.abs().max().item()
        if amax > 0:
            scale = amax / 127.0
            delta_q = torch.clamp(torch.round(all_delta / scale), -127, 127).to(torch.int8)
            delta_bytes = delta_q.numpy().tobytes()
            delta_compressed = _zstd.ZstdCompressor(level=22).compress(delta_bytes)
            log0(f"DELTA: raw_params={all_delta.numel()} int8_bytes={len(delta_bytes)} zstd22={len(delta_compressed)} ({len(delta_compressed)/1024:.0f}KB)")
        del delta_vals, all_delta, raw_cpu, ema_cpu
    ...
    # Temperature sweep
    for T in [0.95, 0.97, 0.98, 0.99, 1.01, 1.02, 1.05]:
        t_vl, t_vb = eval_val_sliding(raw_logits_fn, rank, world_size, device,
            val_tokens_eval, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
            eval_sl, args.eval_stride, eval_batch_seqs=args.eval_batch_seqs, temperature=T)
        log0(f"T={T:.2f} val_bpb={t_vb:.8f}")
    # Extended context test: seq=4096
    if eval_sl <= 2048:
        val_tokens_4k = load_validation_tokens(args.val_files, 4096)
        logits_fn_4k = torch.compile(base_model.forward_logits, dynamic=False)
        warmup_4k = torch.zeros(8, 4096, dtype=torch.int64, device=device)
        with torch.inference_mode(), torch.autocast(device_type="cuda", dtype=torch.bfloat16): _ = logits_fn_4k(warmup_4k)
        vl4k, vb4k = eval_val_sliding(logits_fn_4k, rank, world_size, device,
            val_tokens_4k, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
            4096, 64, eval_batch_seqs=8)
        log0(f"seq4096_stride64 val_bpb={vb4k:.8f}")
