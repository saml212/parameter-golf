...
def ttt_adapt(args, base_model, device, val_tokens, rank=0, world_size=1, log_fn=None):
    ...
    ttt_optim = os.environ.get("TTT_OPTIM", "sgd").lower()
    if ttt_optim == "adam":
        optimizer = torch.optim.Adam(ttt_params, lr=args.ttt_lr, betas=(0.9, 0.95))
    else:
...
def main() -> None:
    ...
    # TTT: Adapt model to validation distribution before sliding eval
    if args.ttt_enabled:
        torch.cuda.synchronize()
        t_ttt = time.perf_counter()
        ttt_adapt(args, base_model, device, val_tokens, rank=rank, world_size=world_size, log_fn=log0)
        torch.cuda.synchronize()
        log0(f"ttt:complete time:{1000.0 * (time.perf_counter() - t_ttt):.0f}ms")
